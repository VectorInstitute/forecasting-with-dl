...
class ConvLSTMEncoder(nn.Module):
    ...
    def _make_lstms(self) -> nn.ModuleDict:
        ...
        for i in range(len(self.cell_channels)):
            ...
            lstms[f"lstm_{i}"] = ConvLSTMLayer(self.dim, in_channels, self.cell_channels[i])
        return lstms

    def forward(self, x: Tensor) -> Dict[str, Tensor]:
        new_states = {}
        # x.shape = torch.Size([128, 10, 1, 64, 64])
        B, _, _, H, W = x.shape
...
class ConvLSTMDecoder(nn.Module):
    ...
    def forward(
        self, states: Dict[str, Tensor]) -> Dict[str, Tensor]:
        preds = []
        for t in range(10):
            for i, (k, lstm) in enumerate(self.lstms.items()):
                state_id = "e" if t == 0 else "d"
                x_in = None if i == 0 else states[f"h_d_lstm_{i - 1}"]
                new_hidden, new_cell = lstm(x_in, states[f"h_{state_id}_{k}"], states[f"c_{state_id}_{k}"])
                states[f"h_d_{k}"] = new_hidden
                states[f"c_d_{k}"] = new_cell
...
class ConvLSTMForecastingNet(nn.Module):
    def __init__(self, 
        ...
        self.encoder_net = ConvLSTMEncoder(64, 1, layer_channels, self.seq_len)
        self.decoder_net = ConvLSTMDecoder(64, 128, layer_channels)
